...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeFormPage() {
        ...
        caseForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                formMessage.textContent = 'Case submitted successfully! You can view it on the dashboard.';
    ...
    function initializeDashboardPage() {
        ...
        const loadCases = async () => {
            try {
                casesTableBody.innerHTML = '<tr><td colspan="5" class="text-center p-8 text-gray-500">Loading cases...</td></tr>';
                const response = await fetch(`${API_URL}/cases`);
                ...
                if (cases.length === 0) {
                    casesTableBody.innerHTML = '<tr><td colspan="5" class="text-center p-8 text-gray-500">No cases have been submitted yet.</td></tr>';
                    return;
                }
                ...
                cases.forEach(caseItem => {
                    ...
                    const statusColor = caseItem.status === 'Analysis Complete' ? 'bg-green-200 text-green-800' : 'bg-yellow-200 text-yellow-800';

                    row.innerHTML = `
                        <td class="px-5 py-5 text-sm"><p class="text-gray-900 whitespace-no-wrap">${caseItem.caseTitle}</p></td>
                        <td class="px-5 py-5 text-sm"><p class="text-gray-900 whitespace-no-wrap">${caseItem.partiesInvolved}</p></td>
                        <td class="px-5 py-5 text-sm"><p class="text-gray-900 whitespace-no-wrap">${new Date(caseItem.submittedAt).toLocaleDateString()}</p></td>
                        <td class="px-5 py-5 text-sm"><span class="relative inline-block px-3 py-1 font-semibold leading-tight ${statusColor} rounded-full"><span class="relative">${caseItem.status}</span></span></td>
                        <td class="px-5 py-5 text-sm text-right">
                            <button data-id="${caseItem._id}" class="view-details-button bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg">View Details</button>
                        </td>
                    `;
        ...
        modalGenerateButton.addEventListener('click', async (e) => {
            ...
            try {
                const response = await fetch(`${API_URL}/cases/${caseId}/generate-judgment`, {
                    method: 'POST'
                });

                if (!response.ok) throw new Error('Failed to generate judgment from server.');
